...
open(Request,Timeout) ->
	...
	{ok,S} ->
	    ...
	    {ok,S};
	Error ->
	    ?log_debug("open failed, reason ~p\n",[Error]),
	    Error
    end.
...
recv_body_data(_Socket, 0, _Fun, _Acc, _Timeout) ->
    ?log_debug("RECV_BODY_DATA: len=0, tmo=~w\n", [_Timeout]),
    {ok, <<>>};
recv_body_data(Socket, Len, Fun, Acc, Timeout) ->
    ?log_debug("RECV_BODY_DATA: len=~p, tmo=~w\n", [Len,Timeout]),    
    rester_socket:setopts(Socket, [{packet,raw},{mode,binary}]),
...
recv_hc(S, R, H, Timeout) ->
	...
	{ok, Hdr} ->
		...
		http_eoh ->
		    ...
		    {ok, R1};
		{http_header,_,K,_,V} ->
		    ?log_debug("HEADER < ~p ~p\n", [K, V]),
		    recv_hc(S,R,set_chdr(K,V,H), Timeout);
		Got ->
		    ?log_debug("HEADER ERROR ~p\n", [Got]),
		    {error, Got}
	    end;
